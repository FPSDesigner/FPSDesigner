...
namespace Editor.Display3D.NotWorking
{
    class CTerrain : IRenderable
    {
        ...
        int nVertices, nIndices, nChunkIndices;
        ...
        public void LoadContent(Texture2D HeightMap, float CellSize, float Height, Texture2D BaseTexture, float TextureTiling, Vector3 LightDirection, GraphicsDevice GraphicsDevice, ContentManager Content)
        {
            ...
            nChunkIndices = (width - 1) * (length - 1) * 6;
            nIndices = nChunkIndices * chunkAmounts;
        ...
        private void createIndices()
        {
            ...
            indicesChunk = new int[chunkAmounts][];
            ...
            {
                int u = 0;
                indicesChunk[chunk] = new int[nChunkIndices];
                for (int x = 0; x < width - 1; x++)
                    for (int z = 0; z < length - 1; z++)
                    {
                        ...
                        indicesChunk[chunk][u++] = upperLeft;
                        indicesChunk[chunk][u++] = upperRight;
                        indicesChunk[chunk][u++] = lowerLeft; 
                        ...
                        indicesChunk[chunk][u++] = lowerLeft;
                        indicesChunk[chunk][u++] = upperRight;
                        indicesChunk[chunk][u++] = lowerRight; 
                    }
            }
        }
        ...
        private void genNormals()
        {
            for (int chunk = 0; chunk < chunkAmounts; chunk++)
            {
                for (int i = 0; i < nChunkIndices; i += 3)
                {
                    // Find the position of each corner of the triangle
                    Vector3 v1 = vertices[chunk][indicesChunk[chunk][i]].Position;
                    Vector3 v2 = vertices[chunk][indicesChunk[chunk][i + 1]].Position;
                    Vector3 v3 = vertices[chunk][indicesChunk[chunk][i + 2]].Position;
                    ...
                    // triangle
                    vertices[chunk][indicesChunk[chunk][i]].Normal += normal;
                    vertices[chunk][indicesChunk[chunk][i + 1]].Normal += normal;
                    vertices[chunk][indicesChunk[chunk][i + 2]].Normal += normal;
                }
        ...
        public void Draw(Matrix View, Matrix Projection, Vector3 cameraPos)
        {
            ...
            for (int chunk = 0; chunk < chunkAmounts; chunk++)
            {
                ...
                if (camera.BoundingVolumeIsInView(boundingBoxChunks[chunk]))
                {
                    ...
                    effect.Parameters["WeightMap"].SetValue(WeightMapChunks[chunkAmountsSide * (chunk % chunkAmountsSide) + chunk / chunkAmountsSide]);
                    //effect.Parameters["chunk"].SetValue(chunk + f);
                    ...
                    */
                    ...
                    GraphicsDevice.DrawIndexedPrimitives(PrimitiveType.TriangleList, 0, 0, nVertices, 0, nChunkIndices);
